...
public class SampleSchemaRegistryClientApp {
	...
	public static final String DEFAULT_SCHEMA_REG_URL = "http://sr.hwx.com:7788/api/v1";
	...
	public void runAvroSerDesApis()  {
		try {
			...
			Object deviceObject;
			deviceObject = createGenericRecordForDevice("/device.avsc");
			...
			//register serde
			String avroSererizerClassName = "com.hortonworks.registries.schemaregistry.serdes.avro.AvroSnapshotSerializer";
			...
			SerDesPair serializerInfo = new SerDesPair(
					...
					"avroSnapshotSerDe",
					avroSererizerClassName,
					avroDeserializerClassName);
			long uid= schemaRegistryClient.addSerDes(serializerInfo);
			String schemaName = schemaMetadata.getName();
			schemaRegistryClient.mapSchemaWithSerDes(schemaName, uid);
			LOG.info("SerDe registered with Avro type, uid: "+uid);
	...
	public void runCustomSerDesApi() throws Exception {
		...
		LOG.info("run Custom Serde- Schema MetadataId: " +v1.getSchemaMetadataId());
